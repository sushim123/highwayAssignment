...
export const signup = async (req: Request, res: Response) => {
  ...
  if (!fullName || !dob || !email) {
    return res
      .status(400)
      .json({ message: "Full name, Date of Birth, and Email are required." });
  }
  ...
  try {
    ...
    const expiresAt = new Date(Date.now() + OTP_EXPIRATION_MINUTES * 60 * 1000); // 5 minutes from now
    pendingOtps.set(email, {
      otp,
      expiresAt,
      type: "signup",
      userData: { fullName, dob, email },
    });
    await sendEmail(
      email,
      "Your Signup OTP",
      `Your OTP for signup is: ${otp}. It is valid for ${OTP_EXPIRATION_MINUTES} minutes.`
    );
    ...
    res
      .status(500)
      .json({ message: "Internal server error during signup request." });
  }
};
...
export const verifyOtpAndSignup = async (req: Request, res: Response) => {
  ...
  if (!otpRecord || otpRecord.type !== "signup") {
    return res.status(400).json({
      message: "No pending signup verification for this email or invalid type.",
    });
  }
  ...
  try {
    ...
    if (!userDataToSave) {
      return res
        .status(500)
        .json({ message: "User data missing for signup completion." });
    }
    ...
    if (error.code === 11000) {
      ...
      return res.status(409).json({
        message: "User with this email already exists after verification.",
      });
    }
    res
      .status(500)
      .json({ message: "Internal server error during signup verification." });
  }
};
export const signin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmail(
      email,
      "Your Signin OTP",
      `Your OTP for signin is: ${otp}. It is valid for ${OTP_EXPIRATION_MINUTES} minutes.`
    );

    console.log(`OTP for signin (${email}): ${otp}`); // Log OTP for testing

    res.status(200).json({
      message: `OTP sent to ${email}. Please verify to complete signin.`,
    });
  } catch (error) {
    console.error("Signin request error:", error);
    res
      .status(500)
      .json({ message: "Internal server error during signin request." });
  }
};
...
export const verifyOtpAndSignin = async (req: Request, res: Response) => {
  ...
  if (!otpRecord || otpRecord.type !== "signin") {
    return res.status(400).json({
      message: "No pending signin verification for this email or invalid type.",
    });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Internal server error during signin verification." });
  }
};
